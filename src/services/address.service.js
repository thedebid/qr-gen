...
async function addAddress(params){
    try{
        ...
        const data = await Address.create({user_id:uid,country:country,state:state,city:city,localAddress:localAddress,postalCode:postalCode});
        return data;
    }catch(err){
...
async function updateAddress(params){
    ...
    try{
        ...
        if(checkData===null){
            ...
            const data = await Address.update({country:country,state:state,city:city,localAddress:localAddress,postalCode:postalCode},{
                where: { id: id }
              })
